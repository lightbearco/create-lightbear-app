 ...
 */
export async function validatePackageManager(packageManager: string): Promise<void> {
  try {
...
export function validateProjectName(name: string): ValidationResult {
  ...
  if (name.length > 214) {
    return { isValid: false, message: "Project name must be less than 214 characters" };
  }
  ...
  if (!/^[a-z0-9-]+$/.test(name)) {
    return { isValid: false, message: "Project name can only contain lowercase letters, numbers, and dashes" };
  }

  if (name.includes("--") || name.startsWith("-") || name.endsWith("-")) {
    return { isValid: false, message: "Project name cannot have consecutive dashes or start/end with dashes" };
  }
 ...
 */
export async function validateDirectoryAvailability(projectPath: string): Promise<boolean> {
